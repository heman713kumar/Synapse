...
export const refineTextWithGemini = async (text: string): Promise<string> => {
  ...
  try {
    ...
    // FIX: As per Gemini API guidelines, response.text directly provides the string output.
    // FIX: Added nullish coalescing operator to handle cases where response.text might be undefined.
...
export const getCollaborationMatches = async (idea: Idea, potentialCollaborators: User[]): Promise<{userId: string, matchScore: number, reason: string}[]> => {
    ...
    **Instructions:**
    ...
    try {
        ...
        // FIX: As per Gemini API guidelines, response.text directly provides the string output. It should be trimmed before parsing as JSON.
        // FIX: Added nullish coalescing operator to handle cases where response.text might be undefined.
