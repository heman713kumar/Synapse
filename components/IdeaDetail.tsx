...
export const IdeaDetail: React.FC<IdeaDetailProps> = ({ ideaId, currentUser, isGuest, setPage, onAchievementsUnlock, onGuestAction }) => {
    ...
    const feedbackSummary = useMemo(() => {
        if (feedbackList.length === 0) return { problem: 0, solution: 0, potential: 0, count: 0 };
        const total = feedbackList.reduce((acc, f) => ({
            problem: acc.problem + f.ratings.problemClarity,
            solution: acc.solution + f.ratings.solutionViability,
            potential: acc.potential + f.ratings.marketPotential,
        }), { problem: 0, solution: 0, potential: 0 });
        const count = feedbackList.length;
        return {
            problem: total.problem / count,
            solution: total.solution / count,
            potential: total.potential / count,
            count
        };
    }, [feedbackList]);
